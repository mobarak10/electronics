            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="d-none mt-2 text-center d-print-block">
                        <h5 class="mb-0 center" style="font-size: 25px">
                            <strong>{{ config('print.print_details.name') }}</strong></h5>
                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="print-none">
                            ...
                            {{-- for collaps search --}}
                            <button class="btn btn-primary print-none" type="button" data-toggle="collapse"
                                    data-target="#collapseSearch" aria-expanded="false" aria-controls="collapseSearch">
                                <i class="fa fa-search"></i>
                            </button>

                            {{-- for print --}}
                            <a href="#" onclick="window.print();" title="Print" class="btn btn-warning print-none"><i
                                    aria-hidden="true" class="fa fa-print"></i></a>
                        </div>
                    </div>
                            ...
                            <form action="{{ route('purchase.index') }}" method="GET">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <select name="condition[party_id]" class="form-control" id="supplier">
                                            ...
                                            @foreach($parties as $party)
                                                <option
                                                    value="{{ $party->id }}" {{ ((request()->condition['party_id'] ?? '') == $party->id) ? "selected" : "" }}>
                                                    {{ $party->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <input type="text" class="form-control" name="condition[voucher_no]"
                                               value="{{ request()->condition['voucher_no'] ?? '' }}"
                                               placeholder="xxxxxxxx" id="voucher_no">
                                    </div>
                            ...
                            <table class="table table-striped table-bordered table-sm">
                                ...
                                @forelse($purchases as $purchase)
                                    <tr>
                                        ...
                                        <td class="text-right">{{ number_format($purchase->grand_total, 2) }}</td> {{--Only for flat discount--}}
                                        ...
                                        <td class="text-right print-none">
                                            ...
                                            <form action="{{ route('purchase.destroy', $purchase->id) }}" method="post"
                                                  class="d-inline"
                                                  onsubmit="return confirm('Are you sure want to delete this purchase?')"
                                                  title="Delete">
                                                @csrf
                                                @method('DELETE')
